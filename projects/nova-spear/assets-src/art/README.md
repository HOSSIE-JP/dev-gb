...
ゲーム中で使用するオリジナルのネイティブ解像度アートです。既存ゲームの画像・ロゴ・フォントを使用していません。

- 作者: NOVA SPEAR project / OpenAI Codex
- ライセンス: **CC0-1.0**。このディレクトリの図案と `../images/*-f*.png` を CC0 1.0 Universal の下で提供します。
- 出典: `build_art.py` に記述した独自のピクセル配列と整数座標図形。
...
`nova-spear-key-art.png` はOpenAI ImageGenで本作向けに生成したオリジナルの宣伝イラストです。ゲーム中の160×144px画面や、ビルドに使うピクセル原画とは別の画像です。上のピクセル原画のCC0表記には含めません。
