...
「新規作品」のテンプレートで **NOVA SPEAR** を選ぶと、この作品の設定・素材・ステージを引き継いだ別作品を作れます。「作品を複製」でも現在の編集内容を継承します。
...
残機は全ステージ共通で5機。被弾すると自機と発射済みの自弾が消え、90ゲームフレーム（60Hz基準で約1.5秒）後に画面下部へ復帰します。その間も敵と背景は進み、射撃できないため得点の機会を失います。復帰後は150ゲームフレームの無敵時間があります。処理落ち中はこれらの実時間が長くなります。AとBを同時に押した場合は集中ショットを優先します。背景の構造物は自機の下にある風景なので、接触してもダメージを受けません。

ボスを倒すと攻撃が止まり、機体の周囲で繰り返し爆発が起こります。途中でボス本体が消え、専用ファンファーレ「Victory Flare」が終わってから暗転して次のステージへ進みます。連続爆発は144ゲーム描画（60Hz基準で約2.4秒）、ファンファーレは約2.7秒です。負荷が高い場合は演出時間が延びます。フェードアウト・インは各24表示フレーム（合計約0.8秒＋読込時間）。演出中は通常のゲーム進行を止めます。エディターのゲーム設定で撃破演出と切替フェードを変更できます。

この作品の制限時間とTIMEカウントダウンは無効です。HUDは最上段1行の `S`（5桁の得点）と `P`（残機）のみ。ボスを倒すまでステージを継続し、時間満了による作戦失敗はありません。プレイ領域は136pxです。

| ステージ | ボス出現 | ボス／HP | 全破壊時の得点上限 |
| --- | ---: | --- | ---: |
| 01 ORBITAL FRONT | 50秒 | TRIDENT／90 | 15,720 |
| 02 IRON DREADNOUGHT | 60秒 | BASTION／140 | 22,060 |
| 03 NOVA REACTOR | 60秒 | HELIX／200 | 24,400 |

出現時刻は60Hzのゲーム更新を基準にしています。機種や負荷により実時間には差があります。敵配置用の内部時刻は保持しますが、制限時間としては使いません。ボス出現直前は通常の敵が途切れ、背景のスクロールが遅くなります。ボスは射撃せず接近した後、HPに応じて攻撃を切り替えます。
...
背景は上から下へ流れます。弾を撃たないHP1の **SPARK** 編隊を、1面に50機、2・3面に各70機配置しています。左右の縦列を時間差で投入し、Aショットで連続撃破できる区間を設けています。SPARKは1機80点、ほかの敵の撃破点は100〜500点。菱形の **NOVA BEACON** は耐久力5の得点用の敵で、破壊すると800点です。拾うアイテムではなく、接触すれば被弾します。1面に3個、2面に4個、3面に5個出現します。
...
作品固有の確認項目は[受入確認](../../docs/nova-spear-validation.md)、負荷の測定は[性能測定](../../docs/nova-spear-performance.md)を参照してください。GitHub Actionsの成功した実行では、`nova-spear-game-boy`からROMと付属文書を取得できます。

## 素材と利用条件

ゲーム中のピクセル原画とキービジュアルは[素材の出典](assets-src/art/README.md)、音楽は[音楽ライセンス](../../engine/caravan/MUSIC-LICENSE.txt)を参照してください。作品全体・コードの再利用条件は[ライセンス案内](../../docs/licensing.md)に記載しています。
