...
弾幕は最大64種類。`aimed-fan`は発射時の自機方向を中心にした扇弾です。`beam`はプレイヤー専用の幅2pxの貫通光線で、押している間、自機のX座標・Y-8の発射口から上端へ伸びます。`interval`はダメージ周期、`damage`は1周期の威力です。`delay=0`・`repeats=0`で使い、移動速度・寿命・角度・発射点の設定は適用しません。旧`laser`は従来どおり独立して飛ぶ弾です。

`beam`がある作品だけ、コンパイル時に全OBJを8×16へ組み替えます。原画、原点、当たり判定は変えず、下端の余りを透明に埋め、上下の8pxタイルを偶数／奇数の順で配置します。GBCパレットも2枚で共通にします。アニメーションのタイル歩幅、OAM入場判定、フォーカス円も同じ配置を使用します。光線用4タイルと最大9個のOAMを常時予約し、毎更新のVRAM転送は追加しません。光線のない作品は8×8のままです。OBJ全体40個・走査線10個という制約は変わりません。[Pan Docs OAM](https://raw.githubusercontent.com/gbdev/pandocs/master/src/OAM.md)
