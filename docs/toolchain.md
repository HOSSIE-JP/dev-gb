...
GitHub ReleasesのZIPを全体展開し `Caravan-Editor.exe` を起動します。ソースの取得・コンパイルは不要です。初回セットアップは固定版のGBDK、Node.js、フォントをローカルに展開し、内蔵エミュレーターのWASMもSHA-256検証後に `.tools/boytacean` へ保存します。Electron自体は配布版に含まれ、再取得しません。通常のゲーム制作ではGit・Pythonを呼び出さないため、それらのインストールも不要です。

「ツール → セットアップ・修復」で失敗後の再実行ができます。外部エミュレーターは同メニューから追加します。再セットアップで既存作品は置換しません。ZIPを更新する場合は新しいフォルダーへ展開し、旧フォルダーの `projects` をバックアップして必要な作品を移してください。

「ツール → ROMから試遊HTMLを書き出す」は選択したROMを単一HTMLに埋め込みます。初回確認は `.cache/editor/html-export-consent-v1.json` に保存します。公開の権利許諾を与えるものではありません。

配布ビルドは `.github/workflows/release.yml` がWindowsで型検査、テスト、パッケージ起動検査を行い、成功時にZIPとSHA-256をGitHub Releasesへ登録します。配布ファイルは未署名です。
