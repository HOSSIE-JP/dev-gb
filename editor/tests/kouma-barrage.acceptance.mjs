...
if(process.argv.includes('--marisa')){const p=game.player.characters.find(p=>p.id==='marisa');assert.ok(p);Object.assign(game.player,{asset:p.asset,weapon:p.weapon,speed:p.speed,focusWeapon:p.focusWeapon,focusSpeed:p.focusSpeed});}
game.player.characters=[];game.ending.characterSlides=[];game.ending.scoreAfter=false;
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 ...
 try{frames(gb,240);gb.key_press(PadKey.Start);frames(gb,2);gb.key_lift(PadKey.Start);gb.key_press(PadKey.B);
  for(let n=0;n<16000;n++){
   ...
   if(t[17]===3||(t[17]===0&&t[16]===2)){ended=true;break;}assert.notEqual(t[17],2,'fixture survives all seven encounters');
   if(t[17]===1&&r[e]===2&&phase>0){
    ...
    let alive=0;for(let s=0;s<64;s++)if(r.readUInt16LE(syms._ce_bg_life-0xc000+s*2)){alive++;if(!baseline)assert.ok(r.readInt16LE(syms._ce_bg_vy-0xc000+s*2)>=0,'actual SM83 shot velocity points down or horizontally');}assert.equal(alive,count);
  ...
  fs.writeFileSync(path.join(out,label+'-timing.json'),JSON.stringify(data,null,2));
  if(!baseline&&mode===GameBoyMode.Cgb)assert.ok(data.every(r=>r.updatesPerSecond>=58&&r.maxFrameGap<=2),'CGB sustained barrage budget, including focused player fire: '+JSON.stringify(data.filter(r=>r.updatesPerSecond<58||r.maxFrameGap>2)));
