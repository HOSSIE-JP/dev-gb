...
test('startup movie persists portable payload and rejects malformed hardware data',()=>{
 ...
 for(const alter of [m=>m.frames[0].attributes=b(240,8),m=>m.frames[0].palettes=b(56,255),m=>m.frames[0].dmg=m.frames[0].dmg.slice(0,-2)+'A=',m=>m.pcm='',m=>m.frames=Array(37).fill(frame)]){g.startupMovie=structuredClone(valid);alter(g.startupMovie);assert.ok(errors().length);}
});
