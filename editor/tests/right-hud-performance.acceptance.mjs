// Matched 40-bullet input: archived v31 engine, current engine at the old layout, and current right HUD.
// node editor/tests/right-hud-performance.acceptance.mjs BASELINE_ENGINE_DIR BASELINE_LIBRARY OUTPUT [--reuse]
...
const root=process.cwd(),lib=createRequire(import.meta.url)('../build/library.cjs'),oldEngine=path.resolve(process.argv[2]),oldLib=createRequire(import.meta.url)(path.resolve(process.argv[3])),out=path.resolve(process.argv[4]),reuse=process.argv.includes('--reuse');fs.mkdirSync(out,{recursive:true});
const g=lib.readGame(root,'star-caravan');g.graze={enabled:true,radius:6,score:1,flashFrames:12};g.name='bg-local';g.stageFade=false;g.timeLimit=false;g.bossCelebration=false;g.player.lives=9;g.player.invulnerability=90;g.player.respawnDelay=12;
...
const reports=[],wide=structuredClone(g);
for(const variant of ['v31-wide','v32-wide','v32-right']){
 Object.assign(g,structuredClone(wide));const compiler=variant==='v31-wide'?oldLib:lib;
 if(variant==='v32-right'){
  const hud=g.screens.find(s=>s.id==='hud');hud.dock='right';hud.rows=18;hud.columns=5;
  hud.items=['TIME','HP','MODE','SCORE','LIFE','BOMB'].flatMap((label,i)=>[{id:'label'+i,text:label,x:0,y:i*3,palette:0,binding:'none'},{id:'value'+i,text:'',x:0,y:i*3+1,palette:0,binding:['bossTime','boss','bossPhase','score','lives','bombs'][i],digits:[2,3,3,5,1,1][i]}]);
  g.player.x=60;g.stages[0].events[0].x=60;
 }
 // Current modes carry the timer; it cannot expire during the 1000-update run.
 if(variant!=='v31-wide')Object.assign(g.bosses[0].phases[0],{timeLimitSeconds:60,score:100,hp:255});
 const fixture=path.join(out,variant),file=path.join(fixture,'projects/bg-local/build/Debug/bg-local.gb');
 if(!reuse&&!(variant==='v31-wide'&&process.argv.includes('--reuse-baseline'))){for(const dir of['engine','.tools/gbdk','.tools/misaki'])fs.cpSync(path.join(root,dir),path.join(fixture,dir),{recursive:true});if(variant==='v31-wide')fs.cpSync(oldEngine,path.join(fixture,'engine/caravan'),{recursive:true});fs.mkdirSync(path.join(fixture,'projects'),{recursive:true});if(fs.existsSync(path.join(fixture,'projects/bg-local')))compiler.saveGame(fixture,'bg-local',g);else compiler.createProject(fixture,'bg-local',g.title,g);compiler.compile(fixture,'bg-local','Debug',()=>{});}
 ...
 for(const mode of[GameBoyMode.Dmg,GameBoyMode.Cgb]){
  const gb=boot(rom,mode),states=[],gaps=[],bgCycles=[];let peak=0,hits=0,priorLives=9;try{
   ...
   for(let n=0;n<1000;n++){
    ...
    for(let y=top>>>3;y<(top+144-lib.hudHeight(g))>>>3;y++)for(let x=0;x<lib.playWidth(g)/8;x++){
 ...
 for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 const gb=boot(rom,mode),costs={cpu:[],flush:[],publish:[]},dmaEndLy=[];
 const bank=()=>{const data=memory(gb).state;for(let p=data.readUInt32LE(data.length-8);p<data.length-8;){const kind=data.toString('ascii',p,p+4),size=data.readUInt32LE(p+4);p+=8;if(kind==='MBC ')for(let k=p;k<p+size;k+=3)if(data.readUInt16LE(k)===0x2000)return data[k+2];p+=size;}return 1;};
 const seek=symbol=>{for(let n=0;n<10000;n++){gb.step_to(symbol&65535);if((symbol>>>16)===bank())return;gb.clock();}throw Error('banked breakpoint not reached');};
 const returnedCost=()=>{const m=memory(gb),regs=gb.registers(),sp=regs.sp,ret=m.ram.readUInt16LE(sp-0xc000);regs.free();let cost=0;for(let n=0;n<100;n++){cost+=gb.clock()+gb.step_to(ret);const r=gb.registers(),done=r.sp===sp+2;r.free();if(done)return cost;}throw Error('unbalanced function return');};
 try{frames(gb,240);gb.key_press(PadKey.Start);frames(gb,4);gb.key_lift(PadKey.Start);gb.step_to(syms._ce_step);
 for(let n=0;n<1000;n++){
  for(const k of [PadKey.Left,PadKey.Right,PadKey.Up,PadKey.Down])gb.key_lift(k);
  if(n>=300&&n<800)gb.key_press([PadKey.Left,PadKey.Up,PadKey.Right,PadKey.Down][Math.floor((n-300)/50)%4]);
  const cpu=returnedCost();gb.clock();seek(syms._ce_bg_flush);const flush=returnedCost();gb.clock();seek(syms._ce_bg_publish);const publish=returnedCost();if(mode===GameBoyMode.Cgb)dmaEndLy.push(memory(gb).ram[syms._ce_bg_dma_end_ly-0xc000]);gb.clock();gb.step_to(syms._ce_step);
  if(n>=120){costs.cpu.push(cpu);costs.flush.push(flush);costs.publish.push(publish);}
 }
 }finally{gb.free();}
 const modeReport=report.modes.find(r=>r.mode===(mode===GameBoyMode.Dmg?'DMG':'CGB'));
 for(const [key,value] of Object.entries(costs))modeReport[key+'MeanTCycles']=value.reduce((a,b)=>a+b)/value.length;
 if(dmaEndLy.length){assert(Math.max(...dmaEndLy)<=153,'GDMA finishes during VBlank');modeReport.dmaEndLyMax=Math.max(...dmaEndLy);}
 }
 reports.push(report);fs.writeFileSync(path.join(out,'results.json'),JSON.stringify(reports,null,2));
}
// The legacy-wide trace must retain collision/motion behavior. New mode scoring does not occur in this route.
for(let i=0;i<2;i++)assert.equal(reports[0].modes[i].hash,reports[1].modes[i].hash,'wide geometry retains every trace, bullet plane and OAM pose');
