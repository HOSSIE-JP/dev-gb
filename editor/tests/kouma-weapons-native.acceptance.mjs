...
const addresses=[t+17,t+2,t+3,s._ce_character,s._ce_state+10,s._ce_pool_counts+3,...Array.from({length:6},(_,i)=>[s._ce_entities+i*25,s._ce_entities+i*25+2]).flat()],hex=n=>n.toString(16),bp=`${hex(published)}///SHOT ${addresses.map(n=>`%(${hex(n)})%`).join(' ')}`,results=[];
for(const mode of ['DMG','CGB'])for(const character of [0,1]){
 ...
 assert.ok(firing.some(r=>r.slice(6).some((v,i,a)=>!(i%2)&&v===3&&a[i+1]===wanted)),'actual player-shot asset');
 ...
 results.push({mode,character,intervals,assetVerified:true,maxShots:Math.max(...firing.map(r=>r[5])),log:path.join(dir,'debugmsg.txt')});console.log(mode+' '+character+' passed');
}
