...
static CE_Entity *boss(void){uint8_t i;for(i=0;i!=ce_used;++i)if(ce_entities[i].kind==CE_BOSS)return &ce_entities[i];return 0;}
 ...
 for(ce_qa_stage=0;ce_qa_stage!=ce_stage_count;++ce_qa_stage)for(ce_qa_route=0;ce_qa_route!=3;++ce_qa_route){
  ...
  for(ce_qa_phase=1;ce_qa_phase!=4;++ce_qa_phase){
   ...
   if(ce_qa_route==1u || (ce_qa_route==2u && ce_qa_phase!=2u)){
    for(i=0;i!=ce_used;++i)if(ce_entities[i].kind==CE_BOSS){ce_damage_actor(i,100);break;}
   }
   ce_step(0);sample(2);
  }
 }
 ...
 try{
  ...
  for(let stage=0;stage<7;stage++)for(let route=0;route<3;route++){
   ...
   for(let phase=1;phase<=3;phase++){
    ...
    for(let i=0;i<39;i++){const p=syms._ce_entities-0xc000+i*25;if(before.ram[p]===2){entity=p;break;}}
