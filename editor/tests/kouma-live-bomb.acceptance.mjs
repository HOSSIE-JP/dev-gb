...
for(const arena of[false,true])for(const mode of(rom[0x143]===0xC0?[GameBoyMode.Cgb]:[GameBoyMode.Dmg,GameBoyMode.Cgb]))for(const character of[0,1]){
 const gb=boot(rom,mode),label=(arena?'boss':'road')+'-'+(mode===GameBoyMode.Dmg?'DMG':'CGB')+'-'+(character?'marisa':'reimu');
 const snap=()=>{let m=memory(gb);if(m.ram[syms._ce_scene-0xc000]===1){settledTrace(gb,syms._ce_trace);m=memory(gb);}const r=m.ram,b=n=>r[syms[n]-0xc000],s=syms._ce_state-0xc000,t=syms._ce_trace-0xc000;let boss,shots=0;for(let i=0;i<(syms._ce_shot_kind?21:39);i++){const p=syms._ce_entities-0xc000+i*25;if(r[p]===2)boss={hp:r[p+3],phase:r[p+4],x:r.readInt16LE(p+12),y:r.readInt16LE(p+14)};if(r[p]===3)++shots;}if(syms._ce_shot_kind)shots=r[syms._ce_pool_counts-0xc000+3];return{m,scene:b('_ce_scene'),ready:r[t]===67&&r[t+1]===69&&!r[t+22],character:b('_ce_character'),battle:b('_ce_battle_mode'),locked:b('_ce_boss_invulnerable'),bombs:b('_ce_bombs'),left:b('_ce_bomb_left'),image:b('_ce_bomb_image'),bullets:b('_ce_bg_count'),tick:r.readUInt16LE(s),x:r.readInt16LE(s+14),y:r.readInt16LE(s+16),camera:r.readUInt16LE(s+4),lives:r[s+19],boss,shots};};
