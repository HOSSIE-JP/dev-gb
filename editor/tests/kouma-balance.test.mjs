...
test('Kouma uses three 100-HP attacks, increasing density ceilings up to 40, downward broad volleys and targeted upper counters',()=>{
 ...
 for(const id of hostile){const p=g.patterns.find(p=>p.id===id);
  for(let sequence=0;sequence<256;sequence++)for(const [dx,dy]of [[0,-120],[0,120],[-120,0],[120,0]]){
   ...
   if(!["aimed","aimed-fan"].includes(p.kind))for(const a of angles)assert.ok(-lib.COS[a]>=0,id+' travels down or horizontally, including when the player moves above the boss');
  }
 }
});
