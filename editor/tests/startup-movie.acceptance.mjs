...
for(const [label,mode]of(rom[0x143]===0xC0?[['CGB',GameBoyMode.Cgb]]:[['DMG',GameBoyMode.Dmg],['CGB',GameBoyMode.Cgb]])){
 ...
 const state=()=>{const {ram,io}=memory(gb),p=(s._ce_shot_kind?s._ce_render_workspace:s._ce_entities)-0xc000+(rom[0x143]===0xC0?0x1000:0);return {scene:ram[s._ce_scene-0xc000],frame:ram[p+11],overruns:ram[p+12],blocks:ram.readUInt16LE(p+4),skipped:ram[p+13],io,music:ram.readUInt16LE(s._ce_music_row-0xc000)};};
 try{
