...
// Usage: node editor/tests/spawn-performance.mjs ROM.gb game.json results.json [fromTick=120] [toTick=1800]
...
const [romPath,gamePath,output,fromArg,toArg]=process.argv.slice(2), fromTick=Number(fromArg??120), toTick=Number(toArg??1800), game=JSON.parse(fs.readFileSync(gamePath));
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 ...
 try{
  ...
  for(let frame=0;frame<20000&&previous<toTick;frame++){
   ...
   if(previous>=fromTick){if(t===previous+1){groups[born?(dead?'both':'create'):(dead?'destroy':'plain')].push(frame-lastFrame);count++;}else excluded++;}
