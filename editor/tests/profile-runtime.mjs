...
const [romPath,out,fromArg="120",toArg="720"]=process.argv.slice(2), syms=symbols(romPath.replace(/\.gb$/,'.map'));
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 ...
 try{
  ...
  for(let i=0;i<250000;i++){
   ...
   if(tick>=Number(toArg))break;if(tick<Number(fromArg))continue;
