...
test('boss arena validates capacity, cut-in art, name, duration and score wait',()=>{
 const g=game(),b=g.bosses[0];b.battle={background:'bg-bullets',maxBullets:128};
 b.phases[0].intro={enabled:true,background:'test-cut-in',spellName:'Spell',seconds:3};
 assert.equal(lib.validate(g).filter(d=>d.severity==='error').length,0);
 for(const mutate of [()=>b.battle.maxBullets=129,()=>b.battle.background='broken',()=>b.phases[0].intro.seconds=0,()=>b.phases[0].intro.spellName='',()=>b.phases[0].intro.background='missing']){
...
test('preview clears bullets and freezes game time for the whole cut-in',()=>{
 ...
 p.intro={enabled:true,background:g.screens[0].background,spellName:'Spell',seconds:3};
 sim.bgShots=[{x:800,y:800,vx:1,vy:1,life:50,damage:1}];
 sim.startIntro(p);const tick=sim.tick,x=sim.playerX;
 for(let n=0;n<180;n++)sim.step(17);
