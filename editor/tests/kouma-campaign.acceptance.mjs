...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){const gb=boot(rom,mode),label=mode===GameBoyMode.Dmg?'DMG':'CGB',cutins=new Set(),scores=new Set(),talks=new Set(),victories=new Set();let peak=0,done=false;
 ...
 for(let f=0;f<16000;f++){const m=memory(gb).ram,byte=n=>m[syms[n]-0xc000],scene=byte('_ce_scene'),stage=m[syms._ce_state-0xc000+18],left=m.readUInt16LE(syms._ce_intro_left-0xc000);peak=Math.max(peak,byte('_ce_bg_count'));
  const trace=m.subarray(syms._ce_trace-0xc000);if(!trace[22]){const shownStage=trace[4];if(trace[17]===5)talks.add(shownStage);if(trace[17]===9)victories.add(shownStage);if(trace[17]===6){assert.ok(victories.has(shownStage),'victory dialogue precedes score for stage '+shownStage);scores.add(shownStage);}}
 ...
 assert.equal(cutins.size,21,'all seven bosses show three phase cards');assert.equal(scores.size,7);assert.equal(victories.size,7);assert.equal(talks.size,7);assert.ok(done);results.push({mode:label,cutins:[...cutins],scoreStages:[...scores],dialogueStages:[...talks],victoryDialogueStages:[...victories],endingReached:done,peakBgBullets:peak});
 }finally{gb.free();}
}
