...
const root=path.resolve(import.meta.dirname,'../..'),lib=createRequire(import.meta.url)('../build/library.cjs'),out=path.join(root,'.cache/kouma-expansion/campaign-qa'),fixture=path.join(out,'fixture');
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){const gb=boot(rom,mode),label=mode===GameBoyMode.Dmg?'DMG':'CGB',cutins=new Set(),scores=new Set(),talks=new Set();let peak=0,done=false;
 ...
 for(let f=0;f<16000;f++){const m=memory(gb).ram,byte=n=>m[syms[n]-0xc000],scene=byte('_ce_scene'),stage=m[syms._ce_state-0xc000+18],left=m.readUInt16LE(syms._ce_intro_left-0xc000);peak=Math.max(peak,byte('_ce_bg_count'));
  ...
  if(scene===7&&left===170){const key=stage+'-'+m[syms._ce_entities-0xc000+4];cutins.add(key);const p=new PNG({width:160,height:144}),rgb=gb.frame_buffer_eager();for(let i=0;i<160*144;i++){p.data.set(rgb.subarray(i*3,i*3+3),i*4);p.data[i*4+3]=255;}fs.writeFileSync(path.join(out,label+'-'+key+'.png'),PNG.sync.write(p));}
