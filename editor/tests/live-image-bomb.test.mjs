...
function game(){const g=lib.readGame(process.cwd(),'touhou-kouma');g.player.bomb.live=true;g.player.bomb.presentation='image';g.timeLimit=false;g.stageFade=false;g.stages.forEach(s=>{s.events=[];s.requireBoss=false;s.clearOnBoss=false;});return g;}
...
test('phase break and final victory wait until the live image releases the BG',()=>{
 const g=game(),s=new lib.Simulation(g);s.spawnActor('rumia','boss',80,36);const b=s.entities.find(e=>e.kind==='boss');b.phase=1;b.hp=20;s.introLeft=0;s.phaseLocked=false;s.step(0);s.step(48);
