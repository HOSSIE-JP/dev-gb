// Real SM83/PPU oracle: 128 separate shots, composite cells, bitplanes, OAM and timing.
...
const scenario=process.argv[3]??'spread';assert.ok(['spread','pairs','motion'].includes(scenario));
...
void ce_mainloop(void) BANKED {
    ...
    ce_reset(0,1); ce_scene=1; ce_load_stage(); ce_battle_mode=2; ce_bg_limit=128;
    ...
    for(i=0;i!=128u;++i)ce_bg_spawn();
    if(ce_bg_count==128u)ce_qa_flags|=1;
    ce_bg_spawn();if(ce_bg_count==128u&&ce_state.dropped==1u)ce_qa_flags|=2;
    ...
    for(i=0;i!=128u;++i) {
        ce_bg_request.x=${scenario==='pairs'?'(5u+((i>>1)&15u)*9u+(i&1u)*2u)*16u':'(7u+(i&15u)*9u)*16u'};
        ce_bg_request.y=${scenario==='pairs'?'(23u+(i>>5)*22u)*16u':'(23u+(i>>4)*11u)*16u'};
        ce_bg_request.vx=${scenario==='motion'?'(i&15u)<8u?8:-8':'0'};
        ce_bg_spawn();
    }
...
function expectedAt(tick){const expected=new Set();for(let i=0;i<128;i++){
 const x=Math.floor(scenario==='pairs'?5+((i>>1)%16)*9+(i%2)*2:7+(i%16)*9+(scenario==='motion'?tick*((i%16)<8?.5:-.5):0))&~1;
 const y=(scenario==='pairs'?23+(i>>5)*22:23+(i>>4)*11)&~1;
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]) {
 ...
 try {
  for(let frame=0;frame<33;frame++) {
   ...
   assert.equal(read('_ce_bg_count'),128,'all 128 shots remain active');
   assert.equal(read('_ce_qa_flags'),31,'capacity, rejected allocation, expiry, offscreen and collision verified by SM83');
  ...
  results.push({mode:mode===GameBoyMode.Dmg?'DMG':'CGB',scenario,bullets:128,verifiedPixels:expectedAt(1).size,framesPerUpdate:mean,updatesPerSecond:59.7275/mean,gaps});
 } finally {gb.free();}
}
