...
const side=process.argv.includes('--side'),first=side?35:16;
const root=path.resolve(import.meta.dirname,'../..'),lib=createRequire(import.meta.url)('../build/library.cjs'),out=path.resolve(process.argv.slice(2).find(a=>!a.startsWith('--'))??(side?'.cache/side-v03/music':'.cache/kouma-v13/music')),work=path.join(out,'fixture');
...
if(side)fs.writeFileSync(path.join(work,'main.c'),fs.readFileSync(path.join(work,'main.c'),'utf8').replaceAll('track = 16','track = 35').replace('track > 34','track > 37'));
...
const tracks=JSON.parse(fs.readFileSync(path.join(root,'engine/caravan/assets-src',side?'side-score.json':'kouma-score.json'))).tracks,syms=symbols(path.join(work,'music.map')),rom=fs.readFileSync(path.join(work,'music.gb')),results=[];
...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 ...
 for(let i=0;i<600&&read().track!==first;i++)frames(gb,1);
 assert.equal(read().track,first,'fixture boots');
 try{for(const t of tracks){
  assert.equal(read().track,t.id);const seen=new Set(),sections=new Set(),chunks=[];let loop=false,ended=false,last=read().row,peak=0,sum=0,samples=0,elapsedFrames=0;
  const ticks=t.speed+(t.speedHalf?.5:0),expectedFrames=t.bars.length*16*ticks;
  gb.audio_buffer_eager(true);
  const capturePcm=()=>{const pcm=gb.audio_buffer_eager(true);for(const n of pcm){peak=Math.max(peak,Math.abs(n));sum+=n*n;samples++;}if(mode===GameBoyMode.Cgb&&(side||process.argv.includes('--all-audio')||[16,18,34].includes(t.id)))chunks.push(Buffer.from(pcm.buffer,pcm.byteOffset,pcm.byteLength));};
  for(let f=0;f<expectedFrames+32;f++){
   ...
   if(s.row<last){loop=true;break;}last=s.row;frames(gb,1);elapsedFrames++;
   if(f%16===15)capturePcm();
  }
  capturePcm(); // Retain the final partial block through the loop boundary.
  ...
  assert.ok(Math.abs(elapsedFrames-expectedFrames)<=2,'loop duration matches exact row clock');
  ...
  results.push({mode:label,id:t.id,title:t.title,bars:seen.size,sections:[...sections],loop,oneShot:ended,expectedFrames,elapsedFrames,seconds:expectedFrames/59.7275,rawPeak:peak,rawRms:Math.sqrt(sum/samples)});
  gb.key_press(PadKey.Right);frames(gb,1);gb.key_lift(PadKey.Right);frames(gb,1);
 }}finally{gb.free();}
 console.log(label+': all '+tracks.length+' tracks, banks, long rows, loops, SFX channels and PCM verified');
}
