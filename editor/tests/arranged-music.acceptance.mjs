...
for(const mode of [GameBoyMode.Dmg,GameBoyMode.Cgb]){
 ...
 try{for(const t of tracks){
  ...
  for(let f=0;f<t.bars.length*16*t.speed+32;f++){
   ...
   if(f%16===15){const pcm=gb.audio_buffer_eager(true);for(const n of pcm){peak=Math.max(peak,Math.abs(n));sum+=n*n;samples++;}if(mode===GameBoyMode.Cgb&&(side||[16,18,34].includes(t.id)))chunks.push(Buffer.from(pcm.buffer,pcm.byteOffset,pcm.byteLength));}
  }
