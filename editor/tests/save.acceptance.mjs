...
test('battery SRAM: real ROM commits, reboot, interrupted writes, corruption and generation wrap on DMG/CGB',{timeout:240000},()=>{
 ...
 try{
  ...
  const g=lib.readGame(root,'nova-spear');g.mode='caravan';g.stageFade=false;g.timeLimit=true;g.bossCelebration=false;g.stageOrder=[g.stages[0].id];g.startStage=g.stages[0].id;
